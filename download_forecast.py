...
def download_forecast_data(dataset_phy, dataset_bio, vars_phy, vars_bio, version, depth, path_prefix):
    today = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    start_date = today - timedelta(days=2)  # Include yesterday and the day before
    end_date = today + timedelta(days=6)    # Include today and the next 6 days
...
def main():
    path_prefix_forecast = "data/baltic_sea/raw/forecast"
    ...
    download_forecast_data(
        ...
        vars_phy, vars_bio, dataset_version, depth, path_prefix_forecast)
