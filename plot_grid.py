...
def create_3d_scatter_plot(mask, show_axis):
    ...
    coords = mask_true.stack(z=('depth', 'latitude', 'longitude')).dropna(dim='z').coords
    ...
    # Define colors
    norm = mpl.colors.Normalize(vmin=depths_valid.min(), vmax=depths_valid.max())
    blues = plt.cm.Blues(norm(depths_valid) * (1 - 0.5) + 0.5)
    n = []
    for b in blues:
...
def main():
    parser = argparse.ArgumentParser(description="Plot bathymetry mask as 3D scatter plot")
    parser.add_argument("--mask", type=str, default="data/mediterranean/static/bathy_mask.nc",
                        help="Path to the bathymetry mask file")
    parser.add_argument("--show_axis", action="store_true", help="Show axis in the plot")
